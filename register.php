...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($customer_id) && !empty($customer_name) && !empty($phone_number)) {
        $conn->begin_transaction();

        try {
            $stmt1 = $conn->prepare("INSERT INTO CUSTOMER (Customer_ID, Customer_Name) VALUES (?, ?)");
            $stmt1->bind_param("is", $customer_id, $customer_name);
            $stmt1->execute();

            $stmt2 = $conn->prepare("INSERT INTO CUSTOMER_PHONE_NO (Customer_ID, Phone_Number) VALUES (?, ?)");
            $stmt2->bind_param("is", $customer_id, $phone_number);
            $stmt2->execute();
...
<head>
    ...
    <title>Register Customer - Modular ERP System</title>
...
<body class="bg-slate-50 text-slate-800 antialiased flex h-screen overflow-hidden">
    <aside class="w-64 bg-slate-900 text-slate-300 flex flex-col shadow-2xl h-full hidden md:flex">
        ...
        <nav class="flex-1 py-6 space-y-2 px-3 overflow-y-auto">
            ...
            <a href="register.php" class="flex items-center gap-3 px-4 py-3 bg-indigo-600 text-white rounded-lg transition-colors">
    ...
    <main class="flex-1 flex flex-col h-full bg-slate-50 overflow-hidden">
        <header class="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 shadow-sm">
            <h2 class="text-2xl font-bold text-slate-800">Registration Module</h2>
        ...
        <div class="flex-1 overflow-y-auto p-8 flex items-start justify-center">
            <div class="w-full max-w-lg bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden mt-10">
                <div class="bg-indigo-600 px-8 py-6 text-white text-center">
                    <h3 class="text-2xl font-bold">New Customer</h3>
                    <p class="text-indigo-100 mt-1 opacity-90">Enter details to add to the system.</p>
                </div>
                ...
                <div class="p-8">
                    ...
                    <form action="register.php" method="POST" class="space-y-6">
                        <div>
                            <label for="customer_id" class="block text-sm font-semibold text-slate-700 mb-2">Customer ID</label>
                            <input type="number" id="customer_id" name="customer_id" required class="w-full px-4 py-3 rounded-lg border border-slate-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-shadow outline-none text-slate-800 bg-slate-50 focus:bg-white" placeholder="e.g. 1001">
                        </div>
                        
                        <div>
                            <label for="customer_name" class="block text-sm font-semibold text-slate-700 mb-2">Customer Name</label>
                            <input type="text" id="customer_name" name="customer_name" required class="w-full px-4 py-3 rounded-lg border border-slate-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-shadow outline-none text-slate-800 bg-slate-50 focus:bg-white" placeholder="e.g. Acme Corp">
                        </div>

                        <div>
                            <label for="phone_number" class="block text-sm font-semibold text-slate-700 mb-2">Phone Number</label>
                            <input type="text" id="phone_number" name="phone_number" required class="w-full px-4 py-3 rounded-lg border border-slate-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-shadow outline-none text-slate-800 bg-slate-50 focus:bg-white" placeholder="e.g. +1 (555) 000-0000">
                        </div>
